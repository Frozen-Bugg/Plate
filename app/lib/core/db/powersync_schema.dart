...
const schema = Schema([
  ...
  // Phase 3. Foods are per-lifter copies rather than a shared library — the
  // real databases are far too big to sync — so every table here is owned.
  Table('foods', [
    Column.text('user_id'),
    Column.text('name'),
    Column.text('brand'),
    ...
    Column.text('barcode'),
    Column.text('basis'),
    Column.real('kcal_per_100'),
    Column.real('protein_per_100'),
    Column.real('carb_per_100'),
    Column.real('fat_per_100'),
    Column.real('fibre_per_100'),
    Column.real('sugar_per_100'),
    Column.real('sat_fat_per_100'),
    Column.real('sodium_mg_per_100'),
    Column.real('serving_g'),
    Column.text('serving_label'),
    Column.text('last_used_at'),
    Column.integer('favourite'),
    ..._timestamps,
  ], indexes: [
    Index('recent', [IndexedColumn.descending('last_used_at')]),
    Index('barcode', [IndexedColumn('barcode')]),
  ]),
  Table('recipes', [
    Column.text('user_id'),
    Column.text('name'),
  ...
  Table('recipe_items', [
    ...
    Index('recipe', [IndexedColumn('recipe_id')]),
  ]),
  Table('meals', [
    Column.text('user_id'),
    Column.text('meal_on'),
    ...
    Index('eaten', [IndexedColumn.descending('meal_on')]),
  ]),
  Table('meal_items', [
    Column.text('user_id'),
    Column.text('meal_id'),
  ...
  Table('nutrition_targets', [
    ...
    Column.integer('training_day_carb_shift_pct'),
    Column.text('source'),
    Column.integer('tdee_kcal'),
    Column.text('notes'),
    ..._timestamps,
  ], indexes: [
    Index('effective', [IndexedColumn.descending('effective_from')]),
  ]),
